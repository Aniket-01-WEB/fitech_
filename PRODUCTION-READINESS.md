...
**Demo accounts with a public password in production** — see SECURITY-AUDIT.md H2. UI gated; the accounts themselves must be deleted or re-passworded in the Supabase dashboard before a real launch. _Status: partially fixed._
...
**Repository clutter** — `.agents/` (41 files), `.mcp.json`, `skills-lock.json`, `memory.md`, `run-dev.bat`, two unused images. Removed and ignored. _Fixed._

**Duplicated constants / loose typing** — see SECURITY-AUDIT.md L4/L5. _Fixed / partially fixed._

**Large files** — `globals.css` (~5.4k lines, several historical "layers" overriding each other), `admin-portal/page.tsx` (836 lines), `PortalContext.tsx` (832 lines). They work and are internally documented; splitting them is a refactor with regression risk and no functional gain, so it was deliberately not done in an audit pass. _Open (informational)._
