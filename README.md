...
**Backend**: live on Render at **https://fitech-02.onrender.com** — verified working end-to-end (Supabase connectivity, R2 presigned uploads, CORS, security headers all live-tested against the deployed instance, not just assumed from config). Render's free tier spins the backend down after 15 minutes of inactivity — the first request after that takes noticeably longer while it wakes up. The scheduled Supabase keepalive job (see [backend/README.md](backend/README.md)) doesn't prevent this — it only pings Supabase itself, on a very different (5-day) cadence, unrelated to Render's own sleep behavior.

**Frontend**: live on Vercel at **https://fintechclub-phi.vercel.app** — currently broken against the live backend, two dashboard fixes needed (neither is something I can do from here — no Vercel/Render dashboard access):

1. **Vercel → Project Settings → Environment Variables**: add `NEXT_PUBLIC_API_URL=https://fitech-02.onrender.com`, then trigger a new deployment (Vercel bakes `NEXT_PUBLIC_*` vars in at build time — setting the variable alone doesn't affect the build that's already live). Confirmed live right now: without this, the deployed build still calls `http://localhost:4000`, which a deployed HTTPS site can't reach at all (blocked outright, not just failing) — every page that needs the backend (events, login, everything) is broken until this redeploy happens.
2. **Render → the backend service → Environment**: update `FRONTEND_ORIGIN` from its current `http://localhost:3000` to `https://fintechclub-phi.vercel.app` (comma-separate both if local dev against the live backend should also keep working — see the CORS note in `backend/src/server.js`), or CORS will reject every request from the live frontend even after step 1.

R2's bucket CORS is already updated to allow `https://fintechclub-phi.vercel.app` (done directly via the R2 API, confirmed).
