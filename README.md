...
Demo accounts (`student@matrix.club` / `admin@matrix.club`, password `MatrixDemo-2026!`) are real, confirmed Supabase Auth users with `admin`/`student` roles granted via SQL — the login page's Quick Demo buttons sign in through the same real flow. A `superadmin@matrix.club` demo account is not yet created (Supabase's built-in email sender hit its rate limit) — grant that role to any account you do create via the SQL in "Security model" above.
