...
**Backend**: live on Render at **https://fitech-02.onrender.com**. **Frontend**: live on Vercel at **https://fintechclub-phi.vercel.app**. Both verified working together end-to-end — a real login round-trip against the deployed instances (not just a config review): Supabase connectivity, R2 presigned uploads, CORS, and security headers all confirmed live.

Render's free tier spins the backend down after 15 minutes of inactivity, and the very first request to a sleeping instance fails to connect outright rather than just being slow. [`.github/workflows/render-keepalive.yml`](.github/workflows/render-keepalive.yml) pings it every 10 minutes so it never actually goes idle — see [backend/README.md](backend/README.md) for details. This is separate from the scheduled Supabase keepalive job in the same file, which pings Supabase itself on a very different (5-day) cadence to stop the *database* project from auto-pausing — two different problems, two different timescales, two different workflows.
