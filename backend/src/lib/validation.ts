...
function describeIssue(issue, fallbackField: string): string {
  ...
  const field = String(path).replace(/_/g, ' ').replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase();
  switch (issue?.code) {
    case 'too_small':
      if (issue.origin === 'string') return issue.minimum === 1 ? `Please enter a ${field}.` : `${capitalize(field)} is too short.`;
      return `${capitalize(field)} is too small.`;
    case 'too_big':
      return issue.origin === 'string' ? `${capitalize(field)} is too long.` : `${capitalize(field)} is too large.`;
    case 'invalid_type':
...
export function validateBody(schema) {
  return (req, res, next) => {
    ...
    if (!result.success) {
      return res.status(400).json({ error: describeIssue(result.error.issues[0], 'request') });
    }
...
export function validateQuery(schema) {
  return (req, res, next) => {
    ...
    if (!result.success) {
      return res.status(400).json({ error: describeIssue(result.error.issues[0], 'query') });
    }
    next();
  };
}
...
const httpsUrl = (max = 2000) =>
  z.string().trim().min(1).max(max).url().refine((u) => /^https?:\/\//i.test(u), { message: 'Links must start with http:// or https://.' });
...
export const noteCreateSchema = z.object({
  ...
  .refine((v) => Boolean(v.external_link || v.r2_key), { message: 'Add a link or upload a file first.' });
