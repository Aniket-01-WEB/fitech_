...
router.get("/", attachSupabase, async (req, res) => {
  const { data, error } = await req.supabase
    .from("events")
    .select(
      "*, created_by_profile:profiles!events_created_by_fkey(email), reviewed_by_profile:profiles!events_reviewed_by_fkey(email)",
    )
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("events", "approved", "event"),
);
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("events", "rejected", "event"),
);
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("events", "pending", "event"),
);
