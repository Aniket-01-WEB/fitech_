...
router.get('/', requireUser, async (req, res) => {
  const { data, error } = await req.supabase.from('notes').select('*').order('created_at', { ascending: false });
...
router.post('/', requireUser, async (req, res) => {
  const { title, domain, description, file_url, external_link } = req.body;
  ...
  const { data, error } = await req.supabase
    .from('notes')
    .insert({ title, domain, description, file_url, external_link })
    .select()
    .single();
