...
router.post(
  ...
  validateBody(adminRequestCreateSchema),
  async (req, res) => {
    const { data, error } = await req.supabase
      .from("admin_requests")
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("admin_requests", "approved", "adminRequest"),
);
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("admin_requests", "rejected", "adminRequest"),
);
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("admin_requests", "pending", "adminRequest"),
);
