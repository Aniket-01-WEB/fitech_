...
router.post(
  ...
  async (req, res) => {
    ...
    if (contentType && !ALLOWED_NOTE_MIME_TYPES.has(contentType)) {
      return res
        .status(400)
        .json({ error: `File type "${contentType}" isn't allowed for notes.` });
    }
    if (fileSize && fileSize > MAX_NOTE_BYTES) {
      return res
        .status(400)
        .json({ error: "File is too large (25MB max for notes)." });
    }
...
router.post(
  ...
  async (req, res) => {
    if (req.body.r2_key && !ownsKey("notes", req.user.id, req.body.r2_key)) {
      return res
        .status(403)
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("notes", "approved", "note"),
);
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("notes", "rejected", "note"),
);
...
router.post(
  ...
  requireUser,
  validateIdParam,
  statusHandler("notes", "pending", "note"),
);
...
router.delete("/:id", requireUser, validateIdParam, async (req, res) => {
  ...
  if (existing?.r2_key) {
    deleteObject(existing.r2_key).catch((err) =>
      console.error("Failed to delete R2 object for note", req.params.id, err),
    );
  }
