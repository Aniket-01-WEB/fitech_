...
describe('profileUpdateSchema', () => {
  it('accepts a valid partial profile update', () => {
    const result = profileUpdateSchema.safeParse({ name: 'Alice', school: 'SOET' });
...
describe('eventCreateSchema', () => {
  it('accepts a valid event with title only', () => {
    const result = eventCreateSchema.safeParse({ title: 'FinTech Summit 2026' });
    expect(result.success).toBe(true);
  });
  ...
  it('rejects unknown fields like status', () => {
    const result = eventCreateSchema.safeParse({ title: 'Test', status: 'approved' });
    expect(result.success).toBe(false);
  });
...
describe('registrationCreateSchema', () => {
  ...
  it('rejects extra fields', () => {
    const result = registrationCreateSchema.safeParse({ event_id: 1, user_id: 'sneaky' });
    expect(result.success).toBe(false);
  });
});
...
describe('adminRequestCreateSchema', () => {
  it('accepts a reason', () => {
    const result = adminRequestCreateSchema.safeParse({ reason: 'I want to help manage events' });
    expect(result.success).toBe(true);
  });
  ...
  it('rejects extra fields like status', () => {
    const result = adminRequestCreateSchema.safeParse({ reason: 'test', status: 'approved' });
    expect(result.success).toBe(false);
  });
});
...
describe('uploadUrlSchema', () => {
  ...
  it('requires fileName', () => {
    const result = uploadUrlSchema.safeParse({ contentType: 'application/pdf' });
    expect(result.success).toBe(false);
  });

  it('rejects negative fileSize', () => {
    const result = uploadUrlSchema.safeParse({ fileName: 'test.pdf', fileSize: -1 });
    expect(result.success).toBe(false);
  });
});
...
describe('MIME type allowlists', () => {
  ...
  it('blocks exe for recordings', () => {
    expect(ALLOWED_RECORDING_MIME_TYPES.has('application/x-msdownload')).toBe(false);
  });
});
