...
describe("sendError", () => {
  ...
  it("forwards P0001 (trigger raise_exception) message", () => {
    ...
    sendError(res, {
      code: "P0001",
      message: "Only a super admin can approve or reject events.",
    });
    expect(res.json).toHaveBeenCalledWith({
      error: "Only a super admin can approve or reject events.",
    });
  });
  ...
  it("detects Supabase unreachable (ENOTFOUND) and returns 503", () => {
    ...
    sendError(res, {
      ...
      details: "ENOTFOUND cpainkjljrjjwzdgdewz.supabase.co",
    });
  ...
  it("returns generic message for unknown errors", () => {
    ...
    sendError(res, {
      code: "UNKNOWN",
      message: "some internal detail with table names",
    });
    expect(res.json).toHaveBeenCalledWith({
