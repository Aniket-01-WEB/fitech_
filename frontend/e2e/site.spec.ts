...
test.describe("public pages", () => {
  for (const path of [
    ...
    test(`${path} renders without runtime errors or horizontal overflow`, async ({
      page,
    }) => {
...
test.describe("auth", () => {
  test("protected dashboards redirect signed-out visitors to /login", async ({
    page,
  }) => {
    for (const path of [
      "/dashboard",
  ...
  test("demo student can sign in and reach the student portal", async ({
    page,
  }) => {
    test.skip(
      process.env.E2E_DEMO_LOGIN !== "true",
      "needs a reachable backend + Supabase; set E2E_DEMO_LOGIN=true",
    );
