...
const KNOWN: Array<[RegExp, string]> = [
  ...
  [
    ...
    "Please confirm your email address first, then sign in again.",
  ],
  [
    /user already registered|already exists|already registered/i,
    "An account with this email already exists. Try signing in.",
  ],
  [
    /password should be at least|password.*too short|at least \d+ characters/i,
  ...
  [
    /token has expired|otp expired|expired/i,
    "That code has expired. Request a new one.",
  ],
  [
    /invalid.*(otp|token|code)|token.*invalid/i,
    "That code is not valid. Check it and try again.",
  ],
  [
    /not authorized|not allowed|permission|forbidden|42501/i,
    "You do not have permission to do that.",
  ],
  [/not found/i, "That item no longer exists."],
  [
    /too long|too large|max(imum)? size|25 ?mb|750 ?mb/i,
    "That file is too large.",
  ],
  [
    /isn'?t allowed for|file type|not allowed/i,
  ...
  [
    /can'?t reach|failed to fetch|network|fetch failed|load failed|unavailable|paused/i,
...
const LOOKS_TECHNICAL =
  /\b(PGRST\d+|\d{5}|status \d{3}|\d{3} ?\(|\bat\s+\w+\s*\(|TypeError|ReferenceError|SyntaxError|undefined|null|\{|\}|\[object|expected string|expected number|Too small|Too big|Required)\b/i;
